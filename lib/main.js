...
function Main(inputTree, localeData) {
  ...
  this.localeData = localeData || null;
...
Main.prototype.processString = function (str) {
  ...
  if (that.localeData) {
    res = str.replace(pathExpr, function() {
      ...
      return (new Function("" +
        "var data = arguments;" +
        "var result = this." + arguments[1] + " || null;" +
        "if (!result) {return 'no translation for "+arguments[1]+"';}" +
        "result = result.replace(/\{(\.)\}+/g, function() {" +
        "var index = parseInt(arguments[1], 10);" +
        "return (data[index] ? '{{' + data[index] + '}}' : arguments[0]);" +
        "});" +
        "return result;" +
